...
public class PolicyIterationAgent extends Agent {
	...
	public void initRandomPolicy()
	{
		...
		for(Game g: policyValues.keySet()) {
			...
			if (!g.isTerminal()) { //makes sure that there are still available moves on the board                                                  
				List<Move> moves = g.getPossibleMoves();
				Move randMove = moves.get(random.nextInt(moves.size()));  // gets a random integer between 0 and moves.size to reference a random index of moves: a random move
				this.curPolicy.put(g, randMove);  
			}
		}
	...
	protected void evaluatePolicy(double delta)
	{
		...
		double lastValue;
		double maxChange = 1;
		int count =0;  // counter for how many times the while loop runs: number of times it evaluates the state value
		Move move;
		
		while (maxChange > delta) {
			
			for (Game board: curPolicy.keySet()) {
				value = 0;
				move = curPolicy.get(board);
				lastValue = policyValues.get(board);
				maxChange = 0;
				...
				for(TransitionProb moveProb: mdp.generateTransitions(board, move)) {                                                 //perform the Q*(s,a) = sum{T(s,a,s')}
					value += (moveProb.prob * (moveProb.outcome.localReward + discount * policyValues.get(moveProb.outcome.sPrime)));
				}	
				if (value > lastValue) {
					if((value - lastValue) > maxChange) {
						maxChange = (value - lastValue);
					}
					policyValues.put(board, value);	
				} 
	...
	protected boolean improvePolicy()
	{
		...
		for (Game board: curPolicy.keySet()) {
			...
			for (Move move: board.getPossibleMoves()) {
				...
				if (value>lastValue){
					curPolicy.replace(board, move);
					policyValues.replace(board,value);
					cCount += 1;
				}
			}	
		}
